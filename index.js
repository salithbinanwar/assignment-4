...
function decreaseSpinner(itemMinus) {
    let minused = Number(itemMinus.value);

    if (!minused || minused < 0) {
...
document.querySelector('.book-now').addEventListener('click', function () {
    ...
    if (!fryingFrom || !fryingTo || !fryingDeparture || !fryingReturn || !firstClassTicket || !economyTicket) {
        document.querySelector('.warning').textContent = alert(' You forgot to fill the input area');
    } else {
